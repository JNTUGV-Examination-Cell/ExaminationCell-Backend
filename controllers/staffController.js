  ...
  try {
    ...
    if (data) {
      ...
      if (userData) {
        ...
        const userExits = await otp.findOne({
    ...
    if (data) {
      ...
      if (userData) {
        ...
        if (otpRecord !== null) {
          if (currentDateTime < new Date(otpRecord.expiry_at)) {
            ...
            res
              ...
              .json({
                ...
                collegeCode: data.staff_college_code,
                role: data.role,
              });
          } else {
            // OTP is expired
          ...
          res
            .status(200)
            .json({ isLogin: false, message: "User entered the wrong OTP" });
        }
      } else {
        res
          .status(200)
          .json({ isLogin: false, message: "No User found with this email" });
      }
    } else {
      res
        .status(200)
        .json({ isLogin: false, message: "No Staff found with this email" });
    }
  } catch (error) {
    console.error(error);
    res
      ...
      .json({
        isLogin: false,
        message: "Error occurred in verifying OTP to user",
      });
  }
};
